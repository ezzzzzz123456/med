...
import { 
  ...
  FaPills, FaBell, FaCheck, FaSave, FaPrescriptionBottle, FaTrash, FaPlus, FaPen, FaFilePrescription, FaInfoCircle, FaMagic, FaFilePdf, FaImage, FaStethoscope 
} from 'react-icons/fa';
...
const Prescription = () => {
  ...
  const [symptoms, setSymptoms] = useState(''); // <--- NEW FEATURE STATE
  ...
  // ---------------------------------------------------------
  // THE REAL BACKEND CONNECTION (Updated with Symptoms)
  // ---------------------------------------------------------
  const startAnalysis = async () => {
    ...
    // Use FormData to send both the image and the text context
    const formData = new FormData();
    formData.append("file", file);
    formData.append("user_symptoms", symptoms); // <--- NEW FEATURE: Sending context
  ...
  const handleSaveAndNew = () => {
    ...
    setFile(null);
    setPreview(null);
    setSymptoms(''); // Reset symptoms for new scan
  ...
  const renderContent = () => {
    switch(step) {
        case 1: 
            return (
                <div className="max-w-4xl mx-auto text-center animate-fade-in-up h-full flex flex-col justify-center relative z-10 py-10">
                    ...
                    <div className="mb-6 inline-flex p-5 rounded-[2rem] bg-white shadow-lg shadow-cyan-100 border border-cyan-50 mx-auto">
                    ...
                    <p className="text-slate-400 mb-8 text-xl font-medium max-w-2xl mx-auto">
                    ...
                    <div className="relative group w-full max-w-2xl mx-auto mb-6">
                        <div className={`
                            border-[5px] border-dashed rounded-[3rem] p-12 transition-all duration-500 relative overflow-hidden flex flex-col items-center justify-center min-h-[300px]
                            ${file 
                            ...
                            {preview ? (
                                <div className="relative z-10 w-full h-full flex flex-col items-center">
                                    <div className="relative group-hover:scale-[1.02] transition-transform duration-500">
                                        ...
                                        <img src={preview} className="h-48 rounded-xl object-contain relative shadow-lg" alt="Rx Preview" />
                                    </div>
                                    <div className="mt-6 flex items-center gap-2 bg-green-50 text-green-700 px-5 py-2 rounded-full font-bold text-sm border border-green-100">
                                        <FaCheckCircle /> Image Loaded Successfully
                                    </div>
                                </div>
                            ) : (
                                <div className="relative z-10 flex flex-col items-center">
                                    <div className="w-24 h-24 bg-slate-50 rounded-full flex items-center justify-center mb-6 group-hover:scale-110 group-hover:bg-cyan-50 transition-all duration-300">
                                        <FaCloudUploadAlt className="text-5xl text-slate-300 group-hover:text-cyan-500 transition-colors" />
                                    </div>
                                    <h3 className="text-xl font-bold text-slate-700 mb-3 group-hover:text-cyan-700 transition-colors">
                                        Drag & Drop or Click
                                    </h3>
                                    <div className="flex gap-4 text-slate-400 text-xs font-semibold mt-2">
                    ...
                    {/* --- NEW FEATURE: SYMPTOM CONTEXT BOX --- */}
                    <div className="w-full max-w-2xl mx-auto mb-8 animate-fade-in text-left">
                        <label className="flex items-center gap-2 text-sm font-bold text-slate-500 uppercase tracking-wider mb-3 px-2">
                            <FaStethoscope className="text-cyan-500" /> Why did you visit the doctor? (Optional)
                        </label>
                        <textarea 
                            value={symptoms}
                            onChange={(e) => setSymptoms(e.target.value)}
                            placeholder="e.g. I have a high fever and sore throat..."
                            className="w-full p-5 bg-white border-2 border-slate-100 rounded-[1.5rem] text-slate-700 shadow-sm focus:outline-none focus:border-cyan-400 focus:ring-4 focus:ring-cyan-500/5 transition-all resize-none min-h-[100px]"
                        />
                        <p className="text-[11px] text-slate-400 mt-2 px-2 flex items-center gap-1 italic">
                           <FaInfoCircle /> Providing context helps the AI explain the medicines more accurately to you.
                        </p>
                    </div>
                    ...
                    <button 
                        ...
                        disabled={!file} 
                        className={`
                            px-16 py-6 rounded-full font-black text-xl shadow-xl transition-all transform hover:-translate-y-1 mx-auto flex items-center gap-3
                            ${file 
        ...
        case 4:
            ...
            return (
                <div className="max-w-5xl mx-auto animate-fade-in-up pt-4 pb-20">
